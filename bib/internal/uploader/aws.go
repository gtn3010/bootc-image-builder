...
type AwsUploader interface {
	...
	Register(name, bucket, key string, shareWith []string, rpmArch string, bootMode, importRole *string, encrypt bool, kmsKey string) (*string, *string, error)
}
...
func UploadAndRegister(a AwsUploader, filename, bucketName, imageName, targetArch string, pbar *pb.ProgressBar, importRole string, encrypted bool, kmsKey string) error {
	...
	ami, snapshot, err := a.Register(imageName, bucketName, keyName, nil, targetArch, &bootMode, aws.String(importRole), encrypted, kmsKey)
